...
class Bsds500Config(tfds.core.BuilderConfig):
    def __init__(self, dmsp_subset=False, **kwargs):
        super(Bsds500Config, self).__init__(
            version=tfds.core.Version("0.2.0"), **kwargs
        )
...
class Bsds500(tfds.core.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        Bsds500Config(name="all", description="Use all kernels.", dmsp_subset=False),
        Bsds500Config(
            name="dmsp",
        ...
        if self.builder_config.dmsp_subset:
            return [
                tfds.core.SplitGenerator(
                    ...
                    gen_kwargs={"images_dir_path": os.path.join(images_path, "val")},
                ),
            ]
        else:
            return [
                tfds.core.SplitGenerator(
                    ...
                    gen_kwargs={"images_dir_path": os.path.join(images_path, "train")},
                ),
                tfds.core.SplitGenerator(
                    name=tfds.Split.VALIDATION,
                    gen_kwargs={"images_dir_path": os.path.join(images_path, "val")},
                ...
                tfds.core.SplitGenerator(
                    ...
                    gen_kwargs={"images_dir_path": os.path.join(images_path, "test")},
                ),
            ]
    ...
    def _generate_examples(self, images_dir_path):
        ...
        for image_file in image_files:
            ...
            if not self.builder_config.dmsp_subset or image_id in DMSP_IMAGES_IDS:
                yield image_id, {"image": os.path.join(images_dir_path, image_file)}
